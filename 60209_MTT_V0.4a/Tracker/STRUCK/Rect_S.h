...
public:
	...
	Rect_S(T xMin, T yMin, T width, T height) :
		m_xMin(xMin),
		m_yMin(yMin),
		m_width(width),
		m_height(height)
	...
	template <typename T2>
	friend std::ostream& operator <<(std::ostream &rOS, const Rect_S<T2>& rRect);
	
	template <typename T2>
	float Overlap(const Rect_S<T2>& rOther) const;
	
	template <typename T2>
	bool IsInside(const Rect_S<T2>& rOther) const;
...
typedef Rect_S<int> IntRect;
typedef Rect_S<float> FloatRect;

template <typename T>
std::ostream& operator <<(std::ostream &rOS, const Rect_S<T>& rRect)
{
...
template <typename T>
template <typename T2>
float Rect_S<T>::Overlap(const Rect_S<T2>& rOther) const
{
...
template <typename T>
template <typename T2>
bool Rect_S<T>::IsInside(const Rect_S<T2>& rOther) const
{
