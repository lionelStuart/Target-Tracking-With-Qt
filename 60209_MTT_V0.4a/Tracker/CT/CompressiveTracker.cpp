...
using namespace cv;
using namespace std;
void myCTTracker::_init(const cv::Mat &_frame,const cv::Rect &_trackWindow)
{
	cv::Mat temp;
	cv::cvtColor(_frame,temp,CV_BGR2GRAY);
	cv::Rect tempRect=_trackWindow;
	CompressiveTracker::init(temp,tempRect);
}
void myCTTracker::_tracking(const cv::Mat &_frame, cv::Rect &_trackWindow)
{
	cv::Mat temp;
	cv::cvtColor(_frame,temp,CV_BGR2GRAY);
	CompressiveTracker::processFrame(temp,_trackWindow);
}
//------------------------------------------------
...
{//特征放宽和权重初始化，
	...
	{//在min和max区间内生成随机数numRect
		...
		for (int j=0; j<numRect; j++)
		{
			...
			//weightTemp取值【0，1】
			weightTemp = (float)pow(-1.0, cvFloor(rng.uniform(0.0, 2.0))) / sqrt(float(numRect));//equation 2
...
{
	int sampleBoxSize = _sampleBox.size();//正负样本方框的个数
...
void CompressiveTracker::classifierUpdate(Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate)
{
	...
	for (int i=0; i<featureNum; i++)
	{
		meanStdDev(_sampleFeatureValue.row(i), muTemp, sigmaTemp);//计算均值和方差
...
{//匹配所有方框
	...
	float pPos;
	float pNeg;
	int sampleBoxNum = _sampleFeatureValue.cols;//检测到的特征矩阵
	...
	for (int j=0; j<sampleBoxNum; j++)
	{
		...
		{
			_radioMax = sumRadio;//找到最小的sumRadio及对应的采样方框
			_radioMaxIndex = j;
		}
	}
	//cout<<_radioMax<<endl;
...
void CompressiveTracker::init(Mat& _frame, Rect& _objectBox)
{
	...
	integral(_frame, imageIntegral, CV_32F);//积分图

	getFeatureValue(imageIntegral, samplePositiveBox, samplePositiveFeatureValue);//用积分图计算正负样本的值
...
void CompressiveTracker::processFrame(Mat& _frame, Rect& _objectBox)
{
	// predict
	//采样方框
	sampleRect(_frame, _objectBox, rSearchWindow,detectBox);//在上一帧周围以SerachWindow为半径搜索获得的采样方框
	integral(_frame, imageIntegral, CV_32F);
	getFeatureValue(imageIntegral, detectBox, detectFeatureValue);//检测的特征数值
	int radioMaxIndex;
	float radioMax;
	radioClassifier(muPositive, sigmaPositive, muNegative, sigmaNegative, detectFeatureValue, radioMax, radioMaxIndex);
	_objectBox = detectBox[radioMaxIndex];//确定新的跟踪位置
