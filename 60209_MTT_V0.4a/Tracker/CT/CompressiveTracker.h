...
private:
	...
	int featureNum;
	vector<vector<Rect>> features;//_object Box 生成-随机方框50*（2~4）
	vector<vector<float>> featuresWeight;//各方框的权重,公式2中的测量矩阵
	int rOuterPositive;
...
public:
	void processFrame(Mat& _frame, Rect& _objectBox);//返回跟踪结果
	void init(Mat& _frame, Rect& _objectBox);
};
class myCTTracker:private CompressiveTracker
{
public:
	myCTTracker():CompressiveTracker(){}
	~myCTTracker(){}
